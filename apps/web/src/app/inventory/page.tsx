...
export default function InventoryPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await api<InventoryRow[]>("/inventory", { token });
      setRows(data);
    } catch (err) {
      setError((err as ApiError).message || "Failed to load inventory");
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  function openSet(row: InventoryRow) {
    setSelected(row);
    setForm.resetFields();
  ...
  async function onAdjust(values: { delta: number; note?: string }) {
    ...
    try {
      ...
      message.error((err as ApiError).message || "Adjust failed");
    } finally {
      setSaving(false);
    }
  }

  async function onSet(values: { quantity: number; note?: string }) {
    if (!token || !selected) return;
    setSaving(true);
    try {
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            render: (status: InventoryRow["stockStatus"]) => (
