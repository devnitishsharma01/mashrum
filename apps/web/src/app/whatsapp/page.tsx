...
export default function WhatsAppPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await api<WhatsAppStatus>("/whatsapp/status", { token });
      setStatus(data);
    } catch (err) {
      setError((err as ApiError).message || "Failed to load WhatsApp status");
    } finally {
      setLoading(false);
    }
  }, [token]);
  ...
  async function onConnect(values: {
    ...
    try {
      ...
      message.error((err as ApiError).message || "Connect failed");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function onDisconnect() {
    ...
    try {
      ...
      message.error((err as ApiError).message || "Disconnect failed");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function onSimulate(values: {
    ...
    try {
      ...
      message.error((err as ApiError).message || "Simulate failed");
    } finally {
      setSimulating(false);
    }
  }
